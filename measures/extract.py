...
#processes,read,tarjan,write,elapsed
config = {
			'seqKey': "NP-00",
			...
			"table":{
				"header": ['Version','Processes','ReadFromFile','Tarjan','WriteToFile','Elapsed','Speedup','Efficiency'],
			},
			"plot":{
	...
	for folder in folders:
		...
		for filename_key in means:
			...
			if config["seqKey"] in filename_key:
				...
				nt = int(splitted_filename[3].replace('.csv', ''))
		...
		plot_filename = joined_path + "/speedup-" + str(size) + ".jpg"
